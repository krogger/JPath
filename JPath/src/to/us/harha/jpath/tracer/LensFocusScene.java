...
public class LensFocusScene implements Scene {
    private ArrayList<TracerObject> objects;

    public LensFocusScene()
    {
        ...
        Material mat_clear_glass = new Material(Vec3f.BLACK, Vec3f.BLACK, 0.1f, 0.9f, 1.5f, 0.0f);
        ...
        Material mat_cyan_light = new Material(new Vec3f(0.25f, 0.5f, 1.0f).scale(160.0f), Vec3f.BLACK);
        ...
        objects.add(new TracerObject(leftWall, mat_white_diffuse));

        Plane frontWall = new Plane(new Vec3f(0.0f, 0.0f, 16.0f), Vec3f.BACK);
        objects.add(new TracerObject(frontWall, mat_white_diffuse));
        ...
        //Add a lens
        Vec3f lensPosition = new Vec3f(0.0f, 3.5f, 0f);
        Vec3f pointTo = largeBall.getPosition().sub(lensPosition).normalize();
        Lens lens = new Lens(lensPosition, Vec3f.DOWN, 2.0f, 0.4f, 0.05f);
